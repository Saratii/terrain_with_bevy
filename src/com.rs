...
const INPUT_SIZE: (u32, u32) = (CHUNK_SIZE as u32, CHUNK_SIZE as u32); //this defines the indices that the compute shader iterates over. 
const OUTPUT_SIZE: (u32, u32) = (1024 as u32, 1 as u32);
const WORKGROUP_SIZE: u32 = 6;

pub fn setup_g(mut commands: Commands, mut images: ResMut<Assets<Image>>) {
    let mut input_texture = Image::new_fill(
        Extent3d {
            width: INPUT_SIZE.0,
            height: INPUT_SIZE.1,
    ...
    input_texture.texture_descriptor.usage = TextureUsages::COPY_DST | TextureUsages::STORAGE_BINDING | TextureUsages::TEXTURE_BINDING;
...
pub struct GameOfLifeImages {
    ...
    pub texture_c: Handle<Image>,
}

#[derive(Resource)]
pub struct GameOfLifeImageBindGroups([BindGroup; 1]);
...
fn prepare_bind_group(
    ...
    let view_c = gpu_images.get(&game_of_life_images.texture_c).unwrap();
