...
pub fn render(
    ...
    for (material_handle, mut grid_transform) in terrain_material_handle.iter_mut() {
        let grid = &mut images.get_mut(&materials.get_mut(material_handle).unwrap().color_map).unwrap().data;
        // if i == 4 { //center center
        //     *grid = chunk_map[flatten_index_standard_grid(&chunk_x_v, &chunk_y_v, CHUNKS_HORIZONTAL as usize)].clone();
        //     grid_transform.translation.x = chunk_x_g as f32 * CHUNK_SIZE;
        //     grid_transform.translation.y = chunk_y_g  as f32 * CHUNK_SIZE;
        // }
        if i == 0 { //top left
            *grid = chunk_map[flatten_index_standard_grid(&(chunk_x_v - 1), &(chunk_y_v + 1), CHUNKS_HORIZONTAL as usize)].clone();
            grid_transform.translation.x = (chunk_x_g as f32 - 1.) * CHUNK_SIZE;
            grid_transform.translation.y = (chunk_y_g as f32 + 1.) * CHUNK_SIZE;
